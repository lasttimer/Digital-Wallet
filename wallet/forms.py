...
class LoginForm(forms.Form):
    phone = forms.CharField(label='',required=True, max_length=25, widget=forms.TextInput(attrs={'placeholder': 'Mobile No.'}))
...
class RegistrationForm(forms.Form): 
    ...
    def clean_phone(self):
        ...
        try:
            ...
            return username
        raise forms.ValidationError('Mobile No already registered!')
...
class SendMoneyForm(forms.Form):
    phone = forms.CharField(label='',required=True,  widget=forms.TextInput(attrs={'placeholder': 'Mobile No.'}))
    ...
    def clean_amount(self):
        if 'phone' in self.cleaned_data:
            ...
            user = self.cleaned_data['phone']
            data = balance.objects.get(username=user)
            var=int(data.balance)
            print (var)
            print (amount)
            if amount>var:
                raise  ValidationError("You have Insufficient Balance for this transaction")
            elif amount<=0:
